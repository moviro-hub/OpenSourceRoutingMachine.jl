...
function geometry_polyline(response::RouteResponse, route_index::Integer = 1)
    ...
    cstr = Error.with_error() do err
        CWrapper.osrmc_route_response_geometry_polyline(response.ptr, Cuint(route_index - 1), Error.error_pointer(err))
    end
...
function waypoint_name(response::RouteResponse, index::Integer)
    @assert index >= 1 "Julia uses 1-based indexing"
    cstr = Error.with_error() do err
        CWrapper.osrmc_route_response_waypoint_name(response.ptr, Cuint(index - 1), Error.error_pointer(err))
    end
    return unsafe_string(cstr)
end
