...
function distance(response::RouteResponse)
    return with_error() do err
        ccall((:osrmc_route_response_distance, libosrmc), Cfloat, (Ptr{Cvoid}, Ptr{Ptr{Cvoid}}), response.ptr, error_pointer(err))
    end
end
...
function duration(response::RouteResponse)
    return with_error() do err
        ccall((:osrmc_route_response_duration, libosrmc), Cfloat, (Ptr{Cvoid}, Ptr{Ptr{Cvoid}}), response.ptr, error_pointer(err))
    end
end
...
function distance_at(response::RouteResponse, route_index::Integer)
    ...
    return with_error() do err
        ccall((:osrmc_route_response_distance_at, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(route_index - 1), error_pointer(err))
    end
end
...
function duration_at(response::RouteResponse, route_index::Integer)
    @assert route_index >= 1 "Julia uses 1-based indexing"
    return with_error() do err
        ccall((:osrmc_route_response_duration_at, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(route_index - 1), error_pointer(err))
    end
end
...
function geometry_coordinate_latitude(response::RouteResponse, route_index::Integer, coord_index::Integer)
    ...
    return with_error() do err
        ccall((:osrmc_route_response_geometry_coordinate_latitude, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(route_index - 1), Cuint(coord_index - 1), error_pointer(err))
    end
end
...
function geometry_coordinate_longitude(response::RouteResponse, route_index::Integer, coord_index::Integer)
    @assert route_index >= 1 && coord_index >= 1 "Julia uses 1-based indexing"
    return with_error() do err
        ccall((:osrmc_route_response_geometry_coordinate_longitude, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(route_index - 1), Cuint(coord_index - 1), error_pointer(err))
    end
end
...
function waypoint_latitude(response::RouteResponse, index::Integer)
    ...
    return with_error() do err
        ccall((:osrmc_route_response_waypoint_latitude, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(index - 1), error_pointer(err))
    end
end
...
function waypoint_longitude(response::RouteResponse, index::Integer)
    @assert index >= 1 "Julia uses 1-based indexing"
    return with_error() do err
        ccall((:osrmc_route_response_waypoint_longitude, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(index - 1), error_pointer(err))
    end
end
...
function step_distance(response::RouteResponse, route_index::Integer, leg_index::Integer, step_index::Integer)
    ...
    return with_error() do err
        ccall((:osrmc_route_response_step_distance, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Cuint, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(route_index - 1), Cuint(leg_index - 1), Cuint(step_index - 1), error_pointer(err))
    end
end
...
function step_duration(response::RouteResponse, route_index::Integer, leg_index::Integer, step_index::Integer)
    @assert route_index >= 1 && leg_index >= 1 && step_index >= 1 "Julia uses 1-based indexing"
    return with_error() do err
        ccall((:osrmc_route_response_step_duration, libosrmc), Cfloat, (Ptr{Cvoid}, Cuint, Cuint, Cuint, Ptr{Ptr{Cvoid}}), response.ptr, Cuint(route_index - 1), Cuint(leg_index - 1), Cuint(step_index - 1), error_pointer(err))
    end
end
