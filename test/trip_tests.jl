...
using OpenSourceRoutingMachine.Trips:
    ...
    TripSource,
    TripDestination,
    TRIP_SOURCE_ANY_SOURCE,
    TRIP_SOURCE_FIRST,
    TRIP_DESTINATION_ANY_DESTINATION,
...
@testset "Trip - Setters and Getters" begin
    ...
    @testset "Roundtrip" begin
        params = TripParams()
        # Default value
        initial_value = get_roundtrip(params)
        @test initial_value isa Bool

        set_roundtrip!(params, true)
        @test get_roundtrip(params) == true

        set_roundtrip!(params, false)
        @test get_roundtrip(params) == false
    ...
    @testset "Source" begin
        params = TripParams()
        # Default value
        initial_source = get_source(params)
        @test initial_source isa TripSource

        set_source!(params, TRIP_SOURCE_ANY_SOURCE)
        @test get_source(params) == TRIP_SOURCE_ANY_SOURCE

        set_source!(params, TRIP_SOURCE_FIRST)
        @test get_source(params) == TRIP_SOURCE_FIRST
    end

    @testset "Destination" begin
        params = TripParams()
        # Default value
        initial_destination = get_destination(params)
        @test initial_destination isa TripDestination

        set_destination!(params, TRIP_DESTINATION_ANY_DESTINATION)
        @test get_destination(params) == TRIP_DESTINATION_ANY_DESTINATION

        set_destination!(params, TRIP_DESTINATION_LAST)
        @test get_destination(params) == TRIP_DESTINATION_LAST
    end
...
@testset "Trip - Query Execution" begin
    @testset "Basic trip query" begin
        ...
        response = trip_response(get_test_osrm(), params)
...
@testset "Trip - Error Handling" begin
    @testset "Invalid coordinates" begin
        ...
        maybe_response = try
            trip_response(get_test_osrm(), params)
        catch e
            @test e isa OSRMError
            nothing
        end
